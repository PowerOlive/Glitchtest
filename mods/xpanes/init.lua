...
local nodes_scout_can_place = {
	"default:torch",
	"backpacks:backpack_leather",
}
for i = 1, #nodes_scout_can_place do
	if not minetest.registered_nodes[nodes_scout_can_place[i]].groups then
		break
	end
	local groups = minetest.registered_nodes[nodes_scout_can_place[i]].groups
	groups.scout_can_place = 1
	minetest.override_item(nodes_scout_can_place[i], {
		groups = groups,
	})
end
minetest.register_on_placenode(function(pos, node, placer, oldnode, itemstack, pointed_thing)
	-- What in the world is minetest.registered_on_placenodes?
	local class = placer:get_meta():get("class")
	if placer and class == "scout" then
		if minetest.get_item_group(node.name, "scout_can_place") > 0 then
			return true
		end
...
for k, v in ipairs(dye.dyes) do
	if v[1] == "white" then
		...
		local c = v[1]:gsub("_", "")
		xpanes.register_pane("pane_" .. v[1], {
			description = v[2] .. " Glass Pane",
			textures = {
				"default_glass.png^[colorize:" .. c,
				"xpanes_pane_half.png^[colorize:" .. c,
				"xpanes_edge.png^[colorize:" .. c,
			},
			inventory_image = "default_glass.png^[colorize:" .. c,
			wield_image = "default_glass.png^[colorize:" .. c,
			sounds = default.node_sound_glass_defaults(),
			groups = {snappy = 2, cracky = 3, oddly_breakable_by_hand = 3},
			recipe = {
