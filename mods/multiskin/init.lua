...
local birthday_suit = {"multiskin_female.png", "multiskin_male.png"}
local default_skin = function(player)
	if player and player:get_meta():get_string("gender") == "male" then
		return birthday_suit[2]
	elseif player and player:get_meta():get_string("gender") == "female" then
		return birthday_suit[1]
	else
		local gender = birthday_suit[math.random(2)]
		if player then
			player:get_meta():set_string("gender", gender:sub(11, -5))
		end
		return gender
	end
end
...
multiskin.update_player_visuals = function(player)
	...
	if player:get_meta():get_string("class") == "node" then
		return
	end
...
minetest.register_chatcommand("gender", {
	...
	func = function(name, param)
		...
		if param == "" then
			return true, "You are now " .. player:get_meta():get_string("gender")
		elseif param == "male" then
			player:get_meta():set_string("gender", "male")
			if player:get_meta():get_string("multiskin_skin") == "multiskin_female.png" then
				multiskin.set_player_skin(player, "multiskin_male.png")
			end
			multiskin.update_player_visuals(player)
			return true, "You are now male."
		else
			player:get_meta():set_string("gender", "female")
			if player:get_meta():get_string("multiskin_skin") == "multiskin_male.png" then
				multiskin.set_player_skin(player, "multiskin_female.png")
			end
			multiskin.update_player_visuals(player)
			return true, "You are now female."
		end
	end,
})
